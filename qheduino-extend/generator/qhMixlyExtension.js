...
function setPinMode(pinMode) {
  ...
  Blockly.Arduino.setups_[pinName] =
    "pinMode(" + pinMode[0] + ", " + pinMode[1] + ");";
}
...
function qhValueToCode(key, defaultVal) {
  return (
    Blockly.Arduino.valueToCode(this, key, Blockly.Arduino.ORDER_ATOMIC) ||
    defaultVal
  );
}
...
defineBlockGenerator("rgb_ultrasonic", function () {
  ...
  let code = `float qh_rgb_getDistance(){
  pinMode(13, OUTPUT); 
  digitalWrite(13, LOW);  
  delayMicroseconds(2);         
